...
NSString *const ooPathSpace =@"OOKit";
NSString *const ooDefaultCachePath =@"AppCache";
...
+ (OOCacheManager *)sharedInstance{
    static OOCacheManager *cacheInstance=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        cacheInstance = [[OOCacheManager alloc] init];
    });
...
- (id)init{
    ...
    if (self) {
        ...
        _operationQueue = dispatch_queue_create("com.dispatch.OOCacheManager", DISPATCH_QUEUE_SERIAL);
        
        [self initCachesfileWithName:ooDefaultCachePath];
...
// Library：存放缓存文件，iTunes会备份此目录，此目录下文件不会在应用退出删除
...
// tmp：存放临时文件，iTunes不会备份和恢复此目录，此目录下文件可能会在应用退出后删除  iPhone在重启时，会丢弃所有的tmp文件。
- (NSString *)tmpPath{
    return NSTemporaryDirectory();
}

- (NSString *)OOKitPath{
    return [[self cachesPath]stringByAppendingPathComponent:ooPathSpace];
}

- (NSString *)OOAppCachePath{
...
- (void)initCachesfileWithName:(NSString *)name{
    ...
    self.diskCachePath =[[self OOKitPath] stringByAppendingPathComponent:name];
...
- (BOOL)diskCacheExistsWithKey:(NSString *)key path:(NSString *)path{
    ...
    BOOL exists =[[NSFileManager defaultManager] fileExistsAtPath:codingPath]&&[self isTimeOutWithPath:codingPath timeOut:timeOut]==NO;
    return exists;
}
...
- (void)storeContent:(NSObject *)content forKey:(NSString *)key isSuccess:(OOCacheIsSuccessBlock)isSuccess{
...
- (void)storeContent:(NSObject *)content forKey:(NSString *)key path:(NSString *)path isSuccess:(OOCacheIsSuccessBlock)isSuccess{
...
- (BOOL)setContent:(NSObject *)content writeToFile:(NSString *)path{
    ...
    if ([content isKindOfClass:[NSMutableArray class]]) {
        ...
        NSAssert(NSStringFromClass([content class]), @"非法的文件内容");
...
#pragma  mark - 获取存储数据
- (void)getCacheDataForKey:(NSString *)key value:(OOCacheValueBlock)value{
    
    [self getCacheDataForKey:key path:self.diskCachePath value:value];
}

- (void)getCacheDataForKey:(NSString *)key path:(NSString *)path value:(OOCacheValueBlock)value{
    if (!key)return;
    dispatch_async(self.operationQueue,^{
        @autoreleasepool {
...
- (NSString *)getCacheSizeUnit {
    
    float size = (float)[self getFileSizeWithpath:self.diskCachePath];
    return [self fileUnitWithSize:size];
}
...
- (void)clearCacheWithTime:(NSTimeInterval)time completion:(OOCacheCompletedBlock)completion{
    [self clearCacheWithTime:time path:self.diskCachePath completion:completion];
}

- (void)clearCacheWithTime:(NSTimeInterval)time path:(NSString *)path completion:(OOCacheCompletedBlock)completion{
    if (!time||!path)return;
    dispatch_async(self.operationQueue,^{
        // “-” time
...
- (void)clearCacheForkey:(NSString *)key completion:(OOCacheCompletedBlock)completion{
    
    [self clearCacheForkey:key path:self.diskCachePath completion:completion];
}

- (void)clearCacheForkey:(NSString *)key path:(NSString *)path completion:(OOCacheCompletedBlock)completion{
    if (!key||!path)return;
    dispatch_async(self.operationQueue,^{
...
- (void)clearCacheForkey:(NSString *)key time:(NSTimeInterval)time completion:(OOCacheCompletedBlock)completion{
    [self clearCacheForkey:key time:time path:self.diskCachePath completion:completion];
}

- (void)clearCacheForkey:(NSString *)key time:(NSTimeInterval)time path:(NSString *)path completion:(OOCacheCompletedBlock)completion{
...
- (void)clearCacheOnCompletion:(OOCacheCompletedBlock)completion{
...
- (void)clearDiskWithpath:(NSString *)path completion:(OOCacheCompletedBlock)completion{
    if (!path)return;
    dispatch_async(self.operationQueue, ^{
